...
Usage:
    ...
    python sync_issues.py --audit-prs        # print suspicious PR report
    python sync_issues.py --noise-report     # print noise candidates
    python sync_issues.py --no-cache         # bypass cache, fetch fresh
    python sync_issues.py --dry-run          # show what would change
"""
...
def _link(number: int, show_hash: bool = False) -> str:
    ...
    label = f"#{number}" if show_hash else str(number)
    return f"[{label}](https://github.com/{REPO}/issues/{number})"
...
def _issue_row(i: Issue) -> str:
    ...
    return (
        f"| {_link(i.number)} | {i.title.replace('|', '\\|')} | {i.author} | "
        f"{i.created_at[:10]} | {labels} | {mods} |"
    )


def _pr_row(pr: PR) -> str:
    linked = ", ".join(_link(n, show_hash=True) for n in pr.linked_issues) or "—"
    return (
        f"| {_link(pr.number, show_hash=True)} | `{pr.branch}` | "
        f"{pr.title.replace('|', '\\|')} | {linked} |"
    )
...
def render_markdown(
    ...
    lines.append(
        "> Generated by `sync_issues.py` — personal triage tool "
        "(not for upstream).\n"
    )
    ...
    for i in sorted(bugs_labeled, key=lambda x: x.number, reverse=True):
        lines.append(
            f"| {_link(i.number)} | {i.title.replace('|', '\\|')} | "
            f"{i.author} | {i.created_at[:10]} | {i.severity} | "
            f"{_module_str(i.modules)} |"
        )
    lines.append("")
    ...
    if dupes:
        ...
        for a, b, r in sorted(dupes, key=lambda x: -x[2]):
            lines.append(f"| {_link(a)} | {_link(b)} | {r:.2f} |")
        lines.append("")
    ...
    for pr in sorted(suspicious_prs, key=lambda x: x.number, reverse=True):
        lines.append(
            f"- **{_link(pr.number, show_hash=True)}** [`{pr.branch}`] "
            f"by @{pr.author} (+{pr.additions}/-{pr.deletions}) — {pr.title}"
        )
